...
本章系统讲解了信用风险评分卡的完整建模框架。学完后，建议按以下三层来掌握：

**必须掌握**

1. **风险量化主线**：PD × LGD × EAD = EL，明确了违约概率、损失率和风险暴露三者的分工。
2. **评分卡建模骨架**：分箱、WOE/IV、逻辑回归、AUC/KS 评估，构成传统评分卡的主流程。
3. **评分输出**：Base-PDO 刻度把违约概率转换为业务可操作的分值，是模型落地的最后一步。

**理解即可**

4. **工业约束**：等频分箱之外，还要满足单调性、可解释性和业务直觉。
5. **类别不平衡**：`class_weight='balanced'` 是简单有效的工业起点，但不代表所有场景都应机械使用。
6. **模型演进**：逻辑回归仍是可解释性的主力，树模型和 XGBoost 则在精度上更强，但治理要求更高。

**实践提醒**

信用风险建模不仅是统计问题，更是业务与监管问题。一个“分数更高”的模型，如果无法解释、无法审计、无法稳定落地，往往并不是更好的模型。
...
!!! note "使用建议"
    建议按“风险量化与分箱 → 建模与评估 → 评分卡落地”顺序完成本章习题。若是本科主线课程，可重点完成 18.1、18.2、18.3；若是研究生课程或风控方向，可进一步完成 18.4、18.5。
...
??? tip "参考思路"
    `class_weight='balanced'` 增大坏客户的损失权重，模型倾向于将更多样本预测为违约，因此召回率↑，但精确率↓（更多误报）。银行视角：漏判一个坏客户（FN）损失的是贷款本金；误判一个好客户（FP）损失的是利息收入和客户关系。在高坏账率或经济下行期，银行更愿意牺牲精确率换取更高召回率。
