...
!!! info "配套代码"
    `notebooks/ch15_high_frequency.ipynb`——使用 `np.random.default_rng(15)` 生成的**合成日内数据**，离线可完整运行，无需真实 Level-2数据源。
...
习题1（参见15.12节）要求将价差翻倍，验证一阶自相关更负。notebook 单元 `c15h-092` 的参考解答：
...
配套 notebook 单元 `c15h-041` 用23 400秒（6.5小时）合成数据绘制特征图，可清晰看到：逐秒（间隔1秒）的 RV 显著高于5分钟（300秒）的 RV，而后者更接近有效价格的真实方差。
...
5. **已实现波动率**：$RV=\sum r_i^2$ 是真实波动率的可观测代理，但抽样过密导致噪声放大（波动率特征图），实务折中于5分钟抽样；
...
**习题1**（对应 notebook 单元 `c15h-092`）：在 bid-ask bounce 演示代码中，将买卖价差从0.02扩大到0.04，重新计算观测成交价收益的一阶自相关。与理论预测（自相关绝对值增大约4倍）相比，数值结果是否一致？尝试给出解释。
...
**习题2**（对应 notebook 单元 `c15h-094`）：对合成的秒级数据，分别用间隔1秒、10秒、60秒、300秒（5分钟）计算 RV，并与有效价格的真实积分方差对比。哪个抽样间隔的 RV 估计最接近真实方差？
...
**习题4**：对于一只相对价差为0.2%、单日成交额为5亿元的 A 股，若需在一天内完成500万元的买入执行，估算穿越买卖价差的显性成本，并说明为何市场冲击成本往往更不可忽视。
...
**习题5**：列举3种 A 股 Level-2数据中常见的异常 Tick 类型，并为每种类型设计一条简洁的 pandas 过滤规则（用代码或伪代码描述）。
