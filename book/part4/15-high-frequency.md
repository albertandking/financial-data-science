...
**习题15.1**（对应 notebook 单元 `c15h-092`）：在 bid-ask bounce 演示代码中，将买卖价差从0.02扩大到0.04，重新计算观测成交价收益的一阶自相关。与理论预测（自相关绝对值增大约4倍）相比，数值结果是否一致？尝试给出解释。
...
**习题15.2**（对应 notebook 单元 `c15h-094`）：对合成的秒级数据，分别用间隔1秒、10秒、60秒、300秒（5分钟）计算 RV，并与有效价格的真实积分方差对比。哪个抽样间隔的 RV 估计最接近真实方差？
