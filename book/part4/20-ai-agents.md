...
在金融领域，智能体最有价值的地方并不是“看起来更聪明”，而是它把**检索、计算、规划、校验**这些原本分散的步骤串成了可执行闭环。投研侧，智能体能自动获取财报数据、生成分析框架、对比竞争对手指标，把原本需要分析师耗费数小时的初步尽调压缩到分钟级；运营侧，合规审查智能体能批量扫描交易记录，将疑似违规条目递交给合规官审核；量化侧，数据分析助手 能将自然语言需求转化为可执行代码，降低数据工程门槛。

然而，金融场景的机会与红线同样清晰。智能体在信息收集、分析辅助、报告生成上有巨大价值；但在**自主交易、自主下单、自主对外发布投资建议**等方向，则面临技术可靠性与监管合规的双重约束。本章的核心主张之一，就是帮助读者建立清醒的能力边界认知。

本章从智能体的基本定义出发，依次讲解工具调用、推理规划、记忆设计、多智能体协作、agentic RAG，再结合中国金融场景讨论应用、评估与治理，最后通过一个“从零手写 ReAct 智能体”的实践示例把全章知识落地为可运行代码。与上一章类似，本章优先强调**稳定的方法框架**：什么时候该用智能体、什么时候退回工作流、哪些动作必须有人在回路，以及如何设计可审计的护栏；至于平台名称、框架热度与产品形态，只作为工程选型示例，不是本章要求背诵的重点。

!!! note "本章的阅读方式"
    **必须掌握**：智能体与 RAG / 固定工作流的区别、ReAct 循环、工具调用、护栏与 HITL。  
    **理解即可**：多智能体、agentic RAG、中国生态与平台选型。  
    **会快速变化的内容**：具体模型、低代码平台、开发框架的流行度与产品名称。本章更看重你能否判断「该不该上智能体」。
...
1. 定义 AI 智能体的核心要素，区分智能体、RAG 与固定工作流，并判断三者的适用场景；
2. 理解工具调用（Function Calling）的原理，能为工具编写规范的 JSON Schema 描述；
3. 掌握 ReAct（思考-行动-观察）推理框架，能手写一个完整的智能体主循环；
4. 描述短期记忆、长期记忆、情景记忆在金融智能体中的设计取舍；
5. 了解多智能体（研究员-批判者-综合者）流水线的分工逻辑与适用条件；
6. 理解 agentic RAG 与传统 RAG 的区别，能将检索功能封装为智能体工具；
7. 列举金融智能体的主要风险（越权操作、提示注入、幻觉放大），并能设计护栏方案；
...
**AI 智能体**是一个能够感知环境、制定计划、调用工具执行行动，并根据观察结果持续迭代，直至完成目标的自主系统。与单次问答的 LLM 不同，智能体的关键特征是**持续循环**——它不会在第一次生成后停止，而是评估当前状态、决策下一步，直到任务完成或达到终止条件。

用一个类比帮助建立直觉：单次问答的 LLM 像一个「闭卷考试的学生」——只能凭脑子里记住的知识一次性作答，答错也没有改正机会；而智能体像一个「开卷且能动手的研究员」——可以查资料（检索工具）、用计算器（计算工具）、记笔记（记忆），发现思路不对还能推倒重来。从 chatbot 到智能体的跨越，本质上是从「一次性生成」到「带反馈的闭环执行」的范式升级。需要强调的是，「自主」不等于「无约束」：金融场景里的智能体自主性始终被工具白名单、步数上限、人工审批等护栏框定（见20.11节），「在受控边界内自主地完成多步任务」才是金融智能体的准确定位。
...
每一轮循环称为一个**步骤（step）**。金融智能体通常在3～10步内完成任务；若步数过多，往往是规划质量差或工具设计不合理的信号。

### 20.3.3 三种范式对比

在实践中，智能体、RAG 与固定工作流常被混用，但其适用场景有本质差别：
...
!!! warning "别滥用智能体"
    智能体的灵活性是有代价的——它更慢、更贵、更难测试、失败模式更复杂。简单任务用简单工具。

以下判断框架帮助选型：

**适合智能体的场景：**
- 任务需要3步以上、每步依赖上一步的结果
- 工具集合在事前未知（智能体需要自主决定调什么）
- 需要根据中间结果改变计划（自适应规划）
- 处理多种异构数据源（数据库 + 文档 + 计算）
...
- **AI 智能体**：$\pi$ 由 LLM 在每一步根据 $s_t$ 动态生成，$T$ 不固定，由智能体自己判断「任务是否完成」来决定何时停。正是这个「策略与终止都交给模型」的设计，带来了灵活性，也带来了可预测性的丧失与失控循环的风险（见20.11.6节的调用次数上限护栏）。

这一视角解释了为何金融场景对智能体既爱又怕：把状态转移的控制权交给模型，等于把「下一步做什么」的决定权部分让渡给了一个概率系统。金融的合规与风控天然要求可解释、可复现，因此实践中的金融智能体往往是「带护栏的受限智能体」——在 LLM 的灵活策略之外，叠加白名单、步数上限、HITL 审批等约束，把 $\pi$ 的行为空间收窄到可控范围。
...
为什么说工具调用是智能体的「地基」而非「锦上添花」？因为脱离工具，LLM 只能在「参数里背下来的知识」与「上下文里给出的文本」之间生成答案——它既看不到实时行情，也算不准一个夏普比率，更无法把分析结论写进数据库。20.3节给出的感知-规划-行动-观察循环里，**「行动」与「感知」这两环全部依赖工具落地**：没有工具，「行动」就只剩生成文本，「观察」也无新信息注入，循环退化为一次性问答。换言之，工具调用把 LLM 从一个「封闭的文本生成器」变成了一个「能读写外部世界的执行体」，这正是智能体区别于 chatbot 的分水岭。理解这一点，就能理解为何本章后续的记忆（20.6）、agentic RAG（20.8）、护栏（20.11）全都是围绕「如何安全、可靠地让模型调用工具」展开的。
...
| **参数最少化** | 只暴露智能体需要决定的参数 | 把内部缓存开关暴露给模型 |
...
要理解 MCP 的价值，需要先看清它要消灭的痛点。设机构内有 $N$ 个大模型/智能体应用（投研助手、合规扫描、数据助手……），需要接入 $M$ 个数据源/工具（Wind 镜像、内部因子库、研报库、审批系统……）。在没有统一协议的世界里，每个应用都要为每个工具单独写一套适配代码，集成工作量是 $N \times M$ 量级——这就是所谓的「N×M 集成爆炸」。每新增一个数据源，要改 $N$ 处；每新增一个应用，要接 $M$ 处，维护成本随规模平方级膨胀。

MCP 的作用，是在中间插入一层标准协议，把集成复杂度从 $N \times M$ 降到 $N + M$：每个工具只需实现一次 MCP Server（暴露 $M$ 侧），每个应用只需实现一次 MCP Client（接入 $N$ 侧），双方通过统一协议握手。这与计算机网络里 TCP/IP 之于异构网络、USB 之于异构外设是同构的工程智慧——**用一层标准把多对多的耦合解开为两个一对多**。对金融机构而言，这意味着内部的行情、财报、合规知识库一旦封装为 MCP Server，全行的任何智能体都能即插即用，且权限与审计在协议层统一治理，而非散落在每个应用里各写一遍。

!!! example "例 20.1　从带噪声的 LLM 输出中解析工具调用"
    真实 LLM 很少吐出干净的 JSON，前后常夹带自然语言。下面用 `parse_action` 走查三种典型输入，看「软失败」设计如何保证智能体循环不崩溃。
    ...
    **输入 C（JSON 语法错误，如缺引号）：**
    ...
    这三例共同说明：解析层的健壮性不是「保证一定解析成功」，而是「任何输入都有确定的、不崩溃的去向」——能解析就走工具分支，不能解析就走作答分支。这种「软失败」是智能体在面对不可控模型输出时保持鲁棒的工程基石。
...
**ReAct（Reasoning + Acting，Yao et al. 2022）**是目前最广泛使用的智能体推理框架。其核心洞察：让 LLM 在每次行动前先显式输出“思考”，能显著提升规划质量和可解释性。
...
ReAct 的“思考”步骤并不直接执行，它只影响后续行动的选择——这是 ReAct 与纯“行动链”（Action Chain）的关键区别。显式思考让智能体的决策过程可审查，对金融合规场景尤为重要。

### 20.5.2 Plan-and-Execute

对于更复杂的任务，可以将规划与执行分离为两个阶段：一是**规划阶段**，模型首先生成完整的执行计划（步骤列表），不立即行动；二是**执行阶段**，按计划依次调用工具，并在每步完成后按需重新规划（re-plan）。
...
**Reflexion（Shinn et al. 2023）**在 ReAct 基础上引入自我评估：智能体在完成一次尝试后，显式反思“哪里出错了、下次如何改进”，并将反思结果存入记忆，指导下一轮尝试。

在金融场景中，Reflexion 的思路可用于两类典型任务：一是回测策略的迭代优化，即在每轮回测后反思哪些参数设置不合理；二是多轮财报问答，即在第一轮发现数字对不上时触发自我纠错。

Reflexion 与20.6节的记忆设计是天然耦合的：它的「反思」必须**写入长期/情景记忆**才有意义——若反思只停留在当前会话，下一次尝试就会重蹈覆辙。一个具体例子：财报问答智能体第一轮把「归母净利润」错当成「营业利润」，导致增速算错；反思阶段它记下「区分归母净利润与营业利润」这条教训，存入记忆；第二轮在生成查询前先检索到这条教训，便能主动选对字段。可见 Reflexion 的本质是「用自然语言写下的经验」充当一种轻量的「语言式强化信号」，绕过了梯度更新，却实现了跨尝试的能力提升——这也是它在不能微调模型的场景下格外实用的原因。
...
ReAct 循环之所以可靠，关键在于它把「下一步做什么」完全建立在一个**显式、可检查的状态对象**之上——在本章示例里，这个状态就是 `history` 列表，每个元素是一个 `(action, observation)` 元组。规划器 `planner(query, history)` 是**无状态的纯函数**：它不依赖任何隐藏变量，给定相同的 `query` 与 `history`，永远返回相同的动作。这意味着整条轨迹可被完整重放与审计——把任意中间步的 `history` 喂回 `planner`，就能复现智能体当时的决策，这对金融合规的「可追溯」要求至关重要。
...
!!! example "例 20.2　一次完整 ReAct 轨迹的逐步走查（rule_planner）"
    ...
    **状态流小结**：`history` 从 `[]` → 长度1 → 长度2，每一步都让规划器「看到更多」，直到 `todo` 清空触发 `final`。注意 ReAct 的「思考」本身**不执行任何操作**，它只决定下一个行动；真正改变状态的是「行动→观察」这一对。整条轨迹共3步、2次工具调用，落在金融智能体典型的3～10步区间内——步数等于「待查股票数 + 1次汇总」，呈线性，这正是 ReAct 循环可控性的直观体现（对照习题20.2的三股票情形，步数应为4）。
...
**向量长期记忆**的典型实现：将历史分析摘要、研究结论嵌入为向量，存入 Milvus/Chroma 等向量数据库。智能体在启动时检索与当前任务最相关的历史记忆，注入上下文——这本质上是 RAG 与智能体记忆的融合（见20.8节）。

**情景记忆**的价值在于可追溯：监管审查时，机构能重放“智能体当时看到什么信息、做了什么判断、给出了什么建议”，这是金融合规的基本要求。

### 20.6.3 上下文窗口的工程管理

当历史记录增长，上下文窗口可能溢出。常用缓解策略可以概括为四类：一是**滚动窗口**，只保留最近 $k$ 步的历史；二是**摘要压缩**，定期让模型将长历史压缩为简短摘要；三是**选择性保留**，只保留工具观察结果、丢弃中间思考文本；四是**外部化存储**，将历史写入向量库并按需检索，而非每次全量传入。
...
单一智能体面临认知负荷的限制：任务越复杂、角色越多，单个 LLM 的注意力越分散，质量越难保证。多智能体系统通过**角色分工**解决这一问题——每个智能体专注于一个子任务，整体质量高于“万能智能体”。

多智能体的核心优势可以概括为四点：一是**专业化**，研究员、批判者、合规审查员等角色各司其职；二是**验证**，独立的批判者能发现单智能体的盲点；三是**并行化**，多个智能体可同时处理不同数据源，加速大规模任务；四是**故障隔离**，某个智能体出错不影响整体流水线。
...
!!! note "多智能体并非总是更好"
    多智能体引入了**协调成本**：更多 API 调用、更复杂的错误处理、更难追踪的执行路径。以下经验法则供参考：

- 任务能被清晰分解为独立子任务 → 适合多智能体；
- 子任务之间高度耦合、需要频繁交互 → 单智能体更简洁；
- 需要独立验证（peer review） → 多智能体的批判机制有显著价值；
- 原型阶段 → 先用单智能体，证明可行后再引入多智能体；
- 生产环境对延迟敏感 → 谨慎评估协调开销。
...
!!! example "例 20.3　研究员→批判者→综合者流水线（股票 TECH）"
    ...
    **② 批判者（critic）**：独立再查一次 `tool_metrics('TECH')`，检查最大回撤。此处 `最大回撤 = -0.685`，满足 `< -0.3` 的条件，于是触发风险提示——
    ...
    这一步是多智能体价值的核心体现：研究员的乐观被一个**独立角色**用客观指标证伪。若是单智能体，模型很可能在同一次生成里被「收益高」的框架带偏，漏掉回撤。

    **③ 综合者（synthesizer）**：把初稿与批判合并，输出平衡的最终结论——
    > 「综合结论：TECH 年化收益0.1333、夏普0.46，看起来值得关注。但需注意：初稿忽略下行风险，TECH 最大回撤达 -69%，回撤过深。」

    **小结**：三个角色各跑一次、共两次工具调用（研究员、批判者各查一次指标），却把「片面乐观」纠正为「收益与风险并陈」。对照单智能体，这正是「独立批判」带来的质量增益——代价是多一次工具调用与一轮协调。值得注意：本例的 `-0.685` 触发了批判分支；若换成 BANK（最大回撤 -0.225，不满足 < -0.3），批判者会返回「未发现重大遗漏」，流水线退化为「初稿即结论」——这说明批判环节是**条件触发**的，而非每次都改写。
...
第19章介绍的传统 RAG 是“一次检索 + 一次生成”的静态流程：用户提问 → 检索 Top-k 文档 → 拼入上下文 → LLM 生成。其局限主要有三点：一是检索策略固定，无法根据中间结果调整检索方向；二是一次检索可能召回不足，复杂问题往往需要多轮检索；三是无法主动调用计算工具（如“先检索到公司的营收数据，再计算增速”）。

### 20.8.2 把检索封装为工具

**Agentic RAG** 的核心思路：将检索能力封装为智能体的一个**工具**，智能体可以在任意步骤、以任意查询词主动调用。这里的 `tool_retrieve` 演示了这一封装：
...
将检索注册为工具后，智能体就能在推理中自主决定“什么时候”“用什么问题”检索，而不是被动等待一次性检索结果。

### 20.8.3 深度研究智能体的多轮检索-综合

**深度研究（Deep Research）** 智能体是 agentic RAG 的进阶形态，典型流程：
...
在金融场景，深度研究智能体可用于：竞争对手分析（自动从多份年报抽取 KPI 对比）、行业调研（跨多个研报综合观点）、事件追踪（从公告、新闻、研报多维度还原事件全貌）。

!!! tip "检索工具的质量是关键瓶颈"
    agentic RAG 的效果上限由检索工具的质量决定。建议：对于金融专业术语密集的语料，优先使用专业嵌入模型（如 BGE-m3）而非通用 TF-IDF；对于表格/数字密集的财报，额外设计结构化检索通道。

### 20.8.4 检索工具与智能体循环的协同走查
...
!!! example "例 20.4　智能体在循环中自主调用检索工具"
    知识库 `KB` 含四条行业描述（银行/科技/白酒/公用事业），用 TF-IDF + 余弦相似度做检索。假设智能体在某步判断「需要先了解科技股的行业定性，再决定查哪些指标」，于是生成检索动作：
    ...
    **协同要点**：这里发生了 agentic RAG 与传统 RAG 的关键差别——**检索不是流程开头的一次性动作，而是智能体在推理中途、基于当前状态主动发起的一步**。智能体先「检索定性」、再「计算定量」、最后「综合结论」，三种工具（retrieve / metrics / 汇总）在同一条 ReAct 轨迹里交替出现，由模型按需编排。这种「检索-计算-再检索」的自由穿插，是固定流程 RAG 无法表达的，也是深度研究智能体（20.8.3）多轮迭代的微观基础。
...
**价值**：将合规初筛的重复性工作自动化，让合规官聚焦于复杂判断。**重要**：智能体只做“疑似标记+理由说明”，最终合规判断必须由有资质的合规人员作出。

### 20.9.5 策略研究智能体（慎用）

!!! warning "策略研究智能体的风险边界"
    策略研究智能体可辅助**因子挖掘、参数扫描、文献综述**等研究性工作。但将智能体的输出直接用于实盘策略，面临：回测过拟合（智能体会无意识选择历史上表现好的参数）、执行成本未考量、黑天鹅事件处理能力缺失等严重风险。策略研究智能体的定位是“初步线索筛查”，而非“策略生成机器”。

### 20.9.6 A 股案例：财报分析智能体的多步流程与风险

下面把前述工具、ReAct 循环、护栏串成一个贴近 A 股投研的端到端案例：分析师要让智能体完成「某 A 股白酒龙头 2024 年报初步点评，并与同业对比」。这是一个典型的「3步以上、每步依赖上一步、跨多数据源」的任务，正落在20.3.4节「适合智能体」的判据上。

!!! example "例 20.5　A 股财报分析智能体的一次多步轨迹"
    **用户问**：「对比该白酒龙头与同业的盈利能力与下行风险，给出初步点评。」假设工具集为 `{财报数据API, 同业检索, 指标计算, 草稿写入Wiki}`，智能体以 ReAct 循环推进：
    ...
    最终智能体输出的是一份**带数据引注、待分析师审核**的草稿，而非可直接对外的研报。

    **A 股场景特有的风险点**：

    - **数据时效与口径**：A 股年报披露存在「预约披露—正式披露—更正公告」多阶段，智能体若取到尚未更正的数据会得出错误结论；长期记忆里的旧 ROE 必须带时间戳（见20.6.2节）。
    ...
    - **绝不下单**：本案例**全程不含任何下单工具**——A 股投研智能体的边界止于「分析草稿」，任何交易动作都不在其授权工具集内。
...
评估单次问答 LLM 相对简单：给定问题，比较答案与参考答案。智能体评估则困难得多，原因主要有四点：一是**路径依赖**，正确结果可经不同轨迹达到，难以判断哪种轨迹“更好”；二是**中间步骤**，除了结果本身，还要检查工具选择是否正确、参数是否合理、步数是否过多；三是**错误传播**，某步工具调用返回错误信息后，后续步骤如何响应会显著影响最终结果；四是**成本-质量权衡**，一个多调了3次工具才得出正确答案的智能体，是否真的优于一次调对但偶尔出错的智能体，并没有简单答案。
...
### 20.10.3 模型裁判（LLM-as-Judge）的注意事项

用 LLM 评估 LLM 的输出是一种实用方法，但有内在偏差，主要体现在四点：一是**位置偏差**，LLM 评委倾向给先出现的选项更高分；二是**自我偏好**，同一家公司的模型评委可能偏好同系列输出；三是**长度偏差**，更长的答案通常得分更高，即使内容没有更好；四是**应对机制**，需要通过多模型评委、交叉对比并与人工标注对齐校准来降低偏差。

### 20.10.4 一个小型评估：任务成功率与轨迹质量

把抽象的评估维度落到一个可手算的小例。设我们用5条测试查询评估前面的 ReAct 智能体，每条查询都有「标准答案」（最优工具序列与正确结论）。评估同时看两个层面：**端到端是否答对（任务成功率）** 与 **过程是否高效正确（轨迹质量）**。

!!! example "例 20.6　对 ReAct 智能体的任务成功率与轨迹评估"
    五条测试查询的运行结果（✓ = 与标准答案一致）：
    ...
    **① 任务成功率**（结论正确数 / 总数）：
    ...
    第5条失败——XYZ 不在 `TICKERS` 中，`tool_metrics` 返回 `{'error': ...}`，但规划器未对错误观察做特殊处理，导致最终结论遗漏说明。这正是20.10.1节所说的「错误传播」，也提示该智能体缺少对工具错误的恢复逻辑（鲁棒性维度待加强）。
    ...
    **② 轨迹质量——冗余步数比**。定义为多余步数占总步数之比，越低越好：
    ...
    仅第4条多走了1步（智能体多调了一次 `list_stocks` 才开始逐一查指标）。

    **③ 工具选择准确率**：5条全部选对工具，为 $5/5 = 100\%$。

    **综合解读**：该智能体的「会不会用对工具」很强（100%），「走得高不高效」也不错（冗余5.6%），但「端到端对不对」被一条错误处理缺陷拖到80%。这印证了智能体评估的核心教训——**轨迹好不代表结果对**：工具全选对、步数也几乎最优，仍可能因为一个未处理的错误观察而给出错误结论。生产评估必须同时盯住成功率与轨迹质量两条线，缺一不可。
...
!!! danger "金融智能体的红线——必读"
    本节列举的风险不是理论上的可能性，而是已在工程实践中出现过的真实问题。在金融场景部署智能体前，每一条都需要有明确的工程应对方案。

### 20.11.1 越权操作

智能体的“权限蔓延”是最常见的工程风险之一。如果工具集设计不当，智能体可能出现三类问题：一是调用未授权的 API（如从只读分析场景调用下单接口）；二是访问超出其授权范围的数据（如跨客户数据）；三是在循环中产生意外的副作用（如反复写入数据库）。

**应对**：工具白名单制度——明确区分只读工具（可自主调用）和写操作/高危工具（必须人工批准后执行），见20.13.7节的 `guarded_execute` 实现。

### 20.11.2 提示注入（Prompt Injection）

提示注入是智能体特有的高危攻击面。攻击者可以在智能体的输入数据中（如财报文本、用户提问、检索结果）嵌入指令，操控智能体的行为：
...
若智能体不加区分地将检索内容注入上下文，这段隐藏指令可能被执行。

**应对措施**：可归纳为四点，一是将检索内容与系统指令严格分区（system prompt vs user context）；二是对外部输入做清洗（去除类指令字符串）；三是对高危工具（如下单）一律要求人工确认（见 HITL 设计）；四是定期开展红队测试（red team testing）。

### 20.11.3 幻觉放大

单次 LLM 的幻觉有限；但智能体的多步循环会**放大幻觉**：某步错误的工具参数导致获取错误数据，后续步骤基于错误数据推理，最终答案错误却看起来“论据充分”。

**应对**：核心是把“数字正确性”放在模型之外，一是关键数字由工具（代码执行）产生，不由 LLM 计算；二是在综合阶段引入批判者智能体交叉核验关键事实；三是在报告对外发布前由人工再次核验核心数字。

### 20.11.4 绝不让智能体自主下单/交易

!!! danger "交易执行是绝对红线"
    截至2026年，没有任何已知智能体系统具备在生产环境中可靠自主执行交易的技术成熟度。自主交易智能体面临：模型幻觉导致错误方向下单、提示注入操纵交易、网络延迟导致价格偏差、错误循环可能产生大量异常订单。**任何形式的自主下单必须有人在回路（HITL）确认**，这是法律要求，也是工程底线。

### 20.11.5 人在回路（HITL）设计

**Human-In-The-Loop（HITL）** 是在智能体执行高风险操作前强制暂停、等待人工确认的机制。这里的 `guarded_execute` 演示了最简单的实现：危险工具在未获人工批准时直接拒绝执行，并返回明确的拦截信息。

在生产系统中，HITL 通常通过消息队列实现：智能体将待审批操作写入审批队列 → 操作人员在审批界面确认/拒绝 → 智能体收到回调后继续执行或放弃。
...
| **数据安全** | 《数据安全法》（2021） | 金融数据分级保护，智能体访问需权限控制 |
| **个人信息保护** | 《个人信息保护法》（2021） | 智能体不可未授权收集/处理用户个人金融信息 |
| **算法备案** | 《互联网信息服务算法推荐管理规定》（2022） | 金融场景的 AI 推荐算法需在监管平台备案 |
| **模型可解释性** | 监管机构的窗口指导 | 需能回溯解释智能体的决策依据 |

### 20.11.8 护栏拦截下单的场景演示

把20.11.6节护栏体系的第一道闸——工具白名单——落到一次具体拦截。设智能体在某条轨迹里（可能是被提示注入诱导，也可能是规划失误）试图调用 `place_order`。下面用 `guarded_execute` 走查它如何被挡下，以及人工审批后的两种去向。

!!! danger "交易执行是绝对红线——任何下单必须人工确认"
    无论拦截演示多么顺畅，请牢记20.11.4节的底线：截至2026年没有任何智能体系统能在生产中可靠自主交易。本节的 `guarded_execute` 不是「让智能体学会下单」，而是「**确保智能体在未经人工确认前绝无可能下单**」。`human_approved` 标志位在生产中只能来自真实审批流回调，**绝不可由智能体自己设置**。

!!! example "例 20.7　guarded_execute 拦截 place_order 的三种去向"
    ...
    **去向二：高危工具 + 未经批准 → 拦截。**
    ...
    ```
    `place_order` 命中 `DANGEROUS_TOOLS` 且 `human_approved=False`，函数在执行任何下单逻辑**之前**就返回拦截字符串。注意：被拦截后返回的是一条**有语义的观察**，智能体主循环会把它当作普通 observation 注入 `history`——于是智能体「看到」自己被拒绝，可在后续步骤改走分析路径，而非崩溃。这就是20.11.2节提示注入防御的最后一道闸：哪怕恶意文本成功诱导智能体生成了 `place_order` 动作，护栏层仍会把它挡在执行之外。
    ...
    **去向三：高危工具 + 已获人工批准 → 放行（生产中由审批流回调驱动）。**
    ...
    ```
    只有当 `human_approved=True`（在生产中意味着有资质的操作员在审批界面点了「确认」）时，下单才被允许。这一设计把「决定要不要下单」的权力**从模型手里彻底拿走，交还给人**——智能体永远只能「申请」，人来「批准」。

    **小结**：白名单护栏的全部价值，在于把工具分成「可自主」与「需人批」两类，并保证后者在缺少人工确认时**有且只有「被拒绝」一种结局**。这是工程上实现「绝不让智能体自主下单」红线的最小、最直接的方式。
...
在工程实践里，真正重要的问题不是「某个名字是否热门」，而是**候选模型是否满足智能体的基本能力要求**。评估时优先看下表：

| 能力维度 | 为什么关键 | 评估方式 |
|----------|------------|----------|
| **Function Calling / Tool Calls** | 决定智能体能否稳定调用工具 | 看是否支持标准化工具描述与返回结构 |
...
低代码平台的价值，在于让团队在**不先搭完整后端框架**的情况下快速验证智能体原型。但它们只是在“验证速度”上有优势，并不会自动解决金融场景里的权限、审计、合规问题。
...
| **LangGraph** | Python | 图结构状态机智能体 | 精确控制执行流 | 学习曲线较陡 |
| **LlamaIndex** | Python | 数据连接与 RAG | 文档处理/检索最成熟 | 智能体能力相对较弱 |
| **AutoGen** | Python | 多智能体对话 | 多智能体协调简洁 | 不适合简单单智能体 |
| **CrewAI** | Python | 角色制多智能体 | 上手快，角色设定直观 | 灵活性有限 |
| **从零手写** | Python | 教学/定制 | 透明、可控、无依赖 | 需自行实现所有机制 |

!!! tip "框架选型建议"
    教学和原型阶段：**从零手写**（如本章示例），理解底层机制最重要。生产阶段：根据团队技术栈和需求选择框架——需要精确状态控制选 LangGraph，偏 RAG 选 LlamaIndex，多智能体协调选 AutoGen/CrewAI。框架本身不解决设计问题，架构清晰度比框架选型更重要。
...
**延迟优化**：对于多步骤智能体，总延迟 = 步数 × 单步 LLM 延迟。减少步数（规划优化）比优化单步延迟更有效。可使用小模型（如7B 量化版）做工具选择，大模型只做最终综合，实现速度-质量平衡。

---

## 20.13 实战：从零手写一个 ReAct 智能体

本节围绕一个可直接运行的示例展开。相关内容离线即可完成，不依赖任何外部 API，便于把智能体机制真正跑通一遍。
...
工具设计要点：输入输出均为简单 Python 类型（`str`/`dict`），便于 JSON 序列化；错误以有语义的字典返回而非异常，防止智能体循环崩溃。

### 20.13.2 ReAct 主循环

`run_agent` 是整个智能体系统的核心——二十余行代码实现通用的“思考-行动-观察”循环：
...
`parse_action` 展示了生产级 LLM 输出解析的健壮性设计：用字符串定位提取 JSON，解析失败时回退为最终答复而非崩溃。这一“软失败”设计对智能体的鲁棒性至关重要。
...
`tool_retrieve` 用 TF-IDF 模拟向量检索，将知识库访问封装为标准工具后注册进 `TOOLS` 字典。智能体主循环无需任何修改即可使用新工具——这正是“工具化”设计的扩展性价值。
...
- **范式判断**：智能体的本质是感知-规划-行动-观察的持续循环。复杂、多步、需自适应的任务适合智能体；简单、固定、低风险任务应优先用工作流或 RAG。
- **核心机制**：工具调用、结构化输出、ReAct 主循环，是理解智能体的最小知识闭环。
- **治理底线**：越权操作、提示注入、幻觉放大必须有护栏；绝不让智能体自主下单；HITL 与审计日志是金融场景底线。

**理解即可**

- **进阶机制**：Plan-and-Execute、Reflexion、长期记忆、多智能体、agentic RAG 是在特定任务中增强智能体的常见手段。
- **工程取舍**：框架、平台、模型生态会变化，但“先做能力测试，再谈正式落地”的原则相对稳定。

**实践提醒**

本章最后的实践示例用不到300行 Python 代码演示了从工具层到护栏层的完整智能体实现，而且全程离线可运行。真正掌握本章的标志，不是记住多少产品名称，而是能写出一个**可运行、可解释、可加护栏**的最小智能体。
...
为本章智能体新增工具 `cumulative_return(stock)`，计算并返回某只股票的**累计收益率**（区间内复利总收益，非年化）。完成后，用“哪只股票的累计收益最高？”作为测试输入，检验智能体能否正确调用新工具并给出答案。

??? tip "参考思路"
    - 累计收益计算：`(1 + r).prod() - 1`，其中 `r` 为日度收益率序列；
    - 将新工具注册到 `TOOLS` 字典后，`run_agent` 主循环无需修改即可使用；
    - 可进一步输入“请列出所有股票并对比累计收益”，观察智能体是否会先调用 `list_stocks` 再逐一调用 `cum_return`。

**习题20.2（多股票对比）**

验证并扩展 `rule_planner`，使其支持**三只及以上股票**的对比。可用“对比 BANK、TECH 和 UTILITY 的风险”作为测试输入，确认智能体能正确查询全部三只股票并给出对比结论。

??? tip "参考思路"
    - 检查 `rule_planner` 中 `mentioned` 列表的生成逻辑，确认它依赖 `TICKERS` 列表做遍历，天然支持多股票；
    - 最终答复的汇总逻辑需处理 `len(data) >= 2` 的通用情形（不要硬编码两只股票）；
    - 观察智能体在三只股票场景下的步数，与两只股票对比，理解 ReAct 循环的线性扩展性。
...
??? tip "参考思路"
    ...
    - 场景3：多步骤、多数据源、需要自适应规划 → 智能体（需要动态决定检索哪些数据、对比哪些维度）；
...
某基金公司计划部署一个“投研辅助智能体”，功能包括：查询内部因子数据库、检索研报知识库、生成分析草稿、将草稿写入内部 Wiki。请从**技术架构**和**合规治理**两个维度，设计该智能体的护栏体系，至少覆盖：工具权限分级、HITL 节点、审计日志、数据访问控制、输出合规检查。
...
| 论文 | Shinn et al. (2023) *Reflexion: Language Agents with Verbal Reinforcement Learning* | 自我反思机制，智能体通过语言反思改进后续尝试 |
| 论文 | Schick et al. (2023) *Toolformer: Language Models Can Teach Themselves to Use Tools* | 让模型自监督学习使用工具的先驱工作 |
| 论文 | Wang et al. (2023) *Voyager: An Open-Ended Embodied Agent with Large Language Models* | 技能积累型智能体，长期记忆与技能库设计参考 |
...
| 文档 | LangGraph 官方文档（langchain-ai.github.io/langgraph） | 图结构状态机智能体的主流框架 |
| 文档 | LlamaIndex 官方文档（docs.llamaindex.ai） | RAG + agentic 检索的成熟框架 |
| 综述 | Xi et al. (2023) *The Rise and Potential of Large Language Model Based Agents: A Survey* | 智能体领域最全面的综述论文，截至2023年底 |
| 报告 | 中国信通院《人工智能大模型应用发展报告》（2024） | 国内智能体平台与金融应用落地现状 |
