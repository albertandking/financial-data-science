...
!!! example "例 11.6　A 股涨跌预测：XGBoost 与逻辑回归 AUC"
...
??? tip "参考思路"
    代入 $p_1 = p_2 = 0.5$，$G = 1 - (0.5^2 + 0.5^2) = 0.5$；$H = -2 \times 0.5 \log_2 0.5 = 1$。两者在极值点（均匀分布时）同时达到最大，但数值尺度不同，实践中影响微小。
...
**习题11.3（Boosting 与 Bagging）**

解释 Boosting 与 Bagging 在“偏差-方差权衡”上的不同路径。若金融预测中发现：模型训练 AUC = 0.72，测试 AUC = 0.53，问题更可能是偏差还是方差？应如何调整？

??? tip "参考思路"
    训练/测试差距大 → 高方差（过拟合）。Boosting 需减少迭代轮数（树数）或加大正则化（减小 `max_depth`、增大 `lambda`、降低 `learning_rate` + 早停）；也可增加训练数据、缩短特征回看窗口、移除低质量特征。
...
??? tip "参考思路"
    混合时需对收益率/特征做截面标准化，避免某只股票的均值漂移主导模型；同时测试集须仍为时间靠后的数据。常见陷阱：同一时刻多支股票的标签高度相关（市场整体涨跌），导致虚高 AUC。
