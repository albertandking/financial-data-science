...
!!! info "配套代码"
    `notebooks/ch08_panel_regression.ipynb`（使用 linearmodels / statsmodels）本章实战部分采用**内置财务面板数据集 `fundamentals`（见附录C数据字典）**，数据生成过程内置已知系数（leverage→ROA = −0.12）与公司固定效应，故可检验 FE 能否还原真实系数。所有代码离线可运行。
