...
!!! info "配套代码"
    ...
    本章实战部分采用 `np.random.default_rng` 生成的**教学模拟面板**，所有代码离线可运行。
...
!!! warning "教学模拟数据说明"
    本节全部数据由 `np.random.default_rng(42)` 生成，为**教学演示专用**，不代表任何真实公司。  
    模拟设计内置"已知真实系数"（$\beta_{\text{leverage}} = -0.15$，$\beta_{\text{size}} = 0.05$），便于验证估计量能否还原真实参数。  
    实战中应使用 Wind、CSMAR 等真实财务数据库的上市公司数据。

### 8.9.1 面板设计

我们生成 100 家公司 × 10 年（2014—2023）的模拟面板，共 1000 个观测：

| 变量 | 含义 | 生成方式 |
|------|------|----------|
| `roa` | 资产回报率（ROA） | 由真实系数 + 个体/时间 FE + 随机误差生成 |
| `leverage` | 资产负债率 | 公司均值 + 年度波动，与个体 FE 正相关 |
| `size` | 公司规模（对数资产） | 公司固定基础 + 随机游走 |
| `alpha_i` | 个体固定效应（不可观测） | $N(0, 0.04^2)$，代表管理层能力等 |
| `lambda_t` | 时间固定效应（不可观测） | 代表宏观年度冲击 |

### 8.9.2 Pooled OLS vs FE vs RE 系数对比

配套 notebook 的实验结果：

| 估计方法 | $\hat{\beta}_{\text{leverage}}$ | $\hat{\beta}_{\text{size}}$ | 真实值 |
|----------|--------------------------------|----------------------------|--------|
| Pooled OLS | 偏离真实（负偏或正偏） | 偏离真实 | $-0.15$, $0.05$ |
| FE（个体） | ≈ $-0.15$ | ≈ $0.05$ | $-0.15$, $0.05$ |
| RE | 介于 OLS 与 FE 之间 | 介于两者之间 | $-0.15$, $0.05$ |

Pooled OLS 偏误来源：杠杆率与个体效应 $\alpha_i$ 负相关（高管理层能力公司 → 高 $\alpha_i$，低杠杆），导致混淆效应。

### 8.9.3 Hausman 检验结果解读

模拟数据中，$\alpha_i$ 刻意设计为与杠杆率相关，因此 Hausman 检验应**拒绝 $H_0$**（p 值 < 0.05），选择 FE。

### 8.9.4 聚类标准误的重要性

模拟数据中，误差项含有序列相关成分。比较结果：

- 普通 FE 标准误：较小，t 统计量偏大（假性显著）
- 聚类 FE 标准误：更大，t 统计量更保守（真实显著水平）
...
在本章模拟面板中，个体效应 $\alpha_i$ 与杠杆率 $\text{leverage}_{it}$ 负相关（$\rho = -0.5$）。  
(a) 预测 Pooled OLS 对 $\beta_{\text{leverage}}$ 的偏误方向（高估负效应还是低估负效应）；  
(b) 跑 Pooled OLS，验证你的预测；  
(c) 计算偏误大小：$\hat{\beta}_{\text{Pooled}} - \beta_{\text{true}}$。  
*参考思路：$\alpha_i$ 与 leverage 负相关，且 $\alpha_i$ 对 ROA 正影响 → Pooled OLS 会把 $\alpha_i$ 的正影响归咎到低杠杆，导致低估杠杆的负效应（即 $|\hat{\beta}| < |\beta_{\text{true}}|$）。*

**习题 8.4**（Hausman 检验解读）  
使用本章模拟数据跑 FE 和 RE，手动计算 Hausman 统计量（只对 `leverage` 这一个系数）：  
...
**习题 8.5**（双向 FE 与时间效应）  
在本章模拟数据中，我们加入了时间固定效应（宏观年度冲击）。  
(a) 比较单向 FE（仅个体效应）和双向 FE（个体+时间）的系数差异；  
