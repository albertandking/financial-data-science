...
!!! example "例8.1：手算 Pooled OLS 偏误的符号与近似量级"
    ...
    这与8.9.2节实验所得 $\hat{\beta}_{\text{Pooled}} \approx -0.40$ 完全吻合：Pooled OLS 把 $\alpha_i$ 对 ROA 的正贡献错误地“记账”到低杠杆头上，从而把杠杆的负效应夸大了约3倍。读者由此可见，“偏误”并非玄学，而是 $\mathrm{Cov}(x,\alpha_i)\neq 0$ 这一项的直接后果。
...
!!! info "内置财务面板数据集 `fundamentals`"
    ...
    **200家公司 × 8年（2018—2025），共1600个观测**（平衡面板）。
...
**习题8.1**（面板结构识别）
给定一个包含50家公司、8年数据的 DataFrame（存在部分缺失），请：
(a) 判断是平衡还是非平衡面板；
