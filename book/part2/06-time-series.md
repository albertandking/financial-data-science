...
!!! example "例 6.5：GARCH(1,1) 一步波动率预测"
    设对某A股指数日收益率（以百分数计）拟合出 GARCH(1,1)：$\omega=0.02$，$\alpha=0.10$，$\beta=0.85$。已知今日（$t-1$ 期）的条件方差 $\sigma_{t-1}^2 = 1.5$（对应日波动率 $\sqrt{1.5}\approx 1.22\%$），且昨日实际冲击 $\varepsilon_{t-1} = -3.0$（即下跌3%，一个较大的负冲击）。求对明日（第 $t$ 期）的条件方差与波动率预测。
    ...
    **第三步：对照长期均衡。** 该模型的无条件方差为
    ...
    即长期日波动率 $\sqrt{0.40}\approx 0.63\%$。当前预测的 $2.195$ 远在长期均衡 $0.40$ 之上，说明市场正处于**高波动区制**；由于 $\alpha+\beta=0.95$，半衰期 $h^*=\ln 0.5/\ln 0.95\approx 13.5$ 天，意味着这次波动抬升大约需要近14个交易日才会回落一半，**持续性极强**——这与A股“一旦放大波动就维持数周”的经验高度吻合。
