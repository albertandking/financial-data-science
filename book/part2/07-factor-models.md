...
!!! example "例 7.3：手算 CAPM 的 beta 与 alpha（时序回归）"
    取 6 个月的月度超额收益（已减去无风险利率，单位：%）。$x$ 为市场超额收益，
    $y$ 为某股票超额收益：
...
!!! example "例 7.4：对例 7.3 的 alpha 做 t 检验"
    沿用例 7.3 的数据与结果（$\hat\alpha=0.131$，$\hat\beta=1.536$，$\bar{x}=1$，
    ...
    **结论：** 例 7.3 中那个「看起来为正」的 $\alpha$ 在统计上与 $0$ 没有区别——
    它完全可能是 $6$ 个样本点的随机波动造成的假象。这给出一条重要纪律：
    **任何 alpha 都要先过 $t$ 检验这一关**；小样本（这里只有 $6$ 个月）几乎注定
    检验功效不足，这也是 7.8.3 节强调「检验 $\alpha$ 通常需要 $5$ 年以上月度数据」
...
上升，从 CAPM 的「最多解释一半」到 FF5 的「九成左右」，说明每一代追加的因子
确实抓住了真实而独立的收益来源。**主线二（没有银弹）**：每个模型都有「遗留问题」
那一列——FF3 漏掉动量、Carhart 的动量因子换手极高难以实施、FF5 又让 HML 变得
可有可无却仍不含动量。**一个有趣的现实是：学术界至今没有一个公认「最优」的模型**，
Carhart 四因子在业绩归因中最常用，FF5 在解释截面收益上更全面，而 CAPM 凭其
...
年化跑赢沪深 $300$ 共 $6\%$，听起来很厉害。但若把它对 FF3 回归后发现
