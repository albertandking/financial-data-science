...
**直觉类比**：简单收益率就像银行存款的“利息报价”——直观、与本金成比例；对数收益率则像物理学里的“连续增长率”——本质是资产价格的连续时间变化速率。日常生活中，我们习惯用简单收益率（“今天涨了 2%”），但在做统计建模时，对数收益率更像“天然无量纲的速度计”，更便于做加法和假设正态分布。
...
!!! example "例 5.1：三日收益序列的简单与对数收益率"
    ...
    **结论**：对数收益率可以直接相加得到多期累计收益，而简单收益率必须连乘；对于日度收益（绝对值 $<3\%$），两者的差异通常在 0.1 个百分点以内，但累积到月度或年度时差异显著放大。
...
- **偏度**反映分布的“左右不对称”。负偏度（左偏）意味着“大亏损虽然少见，但一旦发生就很惨”——就像保险理赔，平时没什么，偶尔一次赔光。正态分布的偏度为 0，是完美对称的。
...
- 超额峰度 $5.0$：比正态分布厚尾得多。正态分布下，超过 $3\sigma$ 的事件概率约为 0.27%；而超额峰度为 5 的分布，对应概率可能高出数倍至十几倍。这意味着以正态分布为基础的风险模型（如参数法 VaR）会**系统性地低估**极端损失。
...
**生活类比**：用波动率度量风险，就像用“气温的离散程度”来衡量“冷不冷”——不区分高温和低温，只看变化幅度。如果一只股票天天上涨 3%，波动率也很高，但没有任何投资者会抱怨；真正令人痛苦的是下行的 3%。这正是下行风险指标出现的动机。
...
**A 股场景类比**：2019–2020 年 A 股科技赛道（如半导体、新能源）呈现”牛市大涨、熊市大跌”的高波动特征。对于这类资产，夏普比率会因上涨带来的高波动而被压低，但索提诺比率会更”公平”地只惩罚亏损部分，给出更高的评分——这正是索提诺比率在主动管理领域受到欢迎的原因。
...
!!! example "例 5.4：最大回撤的手动计算"
    设某策略连续 6 日的净值序列为：100、105、103、108、99、102。
    ...
    **直觉**：投资者若在 Day 3 买入（以为净值会继续创新高），Day 4 则账面亏损 $8.33\%$，处于“水下”状态，需要净值反弹至 108 才能“回到水面”。
...
!!! example "例 5.5：用历史模拟法手算 95% VaR"
    假设我们有过去 40 个交易日的日收益率（单位：%），将其升序排列，最差的前 4 个数据如下：

    | 排名（升序） | 日收益率 | 说明 |
    |---|---|---|
    | 第 1 名（最差） | $-3.8\%$ | 极端亏损日 |
    | 第 2 名 | $-2.5\%$ | **5% 分位数（$0.05 \times 40 = 2$）** |
    | 第 3 名 | $-2.1\%$ | |
    | 第 4 名 | $-1.9\%$ | |
...
2015 年 6 月至 8 月的 A 股股灾为理解 VaR 局限提供了极佳的真实案例。

**背景**：沪深 300 指数在 2015 年 6 月 12 日至 8 月 26 日约 50 个交易日内累计下跌约 **45%**，期间多日出现全市场超 1500 只股票跌停的极端情况。

**VaR 的失效表现**（以下数字为基于该时段数据的合理示意值）：

| 风险指标 | 基于 2014 年数据的事前预测 | 2015 年股灾期间的实际情况 |
|---|---|---|
| 95% 单日 VaR（历史法） | $\approx 1.5\%$ | 有 **26 天**（超 50% 的交易日）单日跌幅超过 1.5% |
| 99% 单日 VaR（历史法） | $\approx 2.8\%$ | 有多日单日跌幅超过 **8%**（触及跌停板 10%） |
| 95% 单日 ES（历史法） | $\approx 2.1\%$ | 超 VaR 日的平均亏损约 $4\%$，远超预测的 2.1% |
| 最大单日跌幅 | 预测极端情景约 4% | 2015-08-24（”黑色星期一”）单日跌幅约 **8.87%** |

（注：以上数字为基于公开市场数据的合理示意值，具体数值可能因计算口径略有差异。）

**为何 VaR 完全失效？**

1. **体制转换（Regime Change）**：2014 年数据在牛市环境中收集，不包含熊市尾部信息。基于该数据的 VaR 本质上是”晴天模型”，无法预测”暴风雨”。
2. **流动性危机**：股灾期间大量股票触及跌停板，买方流动性瞬间消失，实际卖出价格远差于理论价格，损失被放大。
3. **相关性骤升**：平时各行业相关系数约 0.3–0.5，股灾期间近乎全部变为 1，组合的分散化优势消失殆尽。
