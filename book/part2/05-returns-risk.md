...
!!! info "配套代码"
    本章示例可在配套示例 中运行。主体实验依赖内置数据，离线即可完成。VaR、ES 与索提诺比率等指标会在 示例中逐步实现。
...
本章系统构建了收益率与风险度量的理论与实践框架。学完后，建议按以下三层来掌握：

**必须掌握**

1. **收益率表示**：简单收益率更适合组合加权，对数收益率更适合时间累加与建模；二者不可长期混用。
2. **年化换算**：收益的复利换算与波动率的 $\sqrt{252}$ 缩放，是金融报表和回测结果解释的基础。
3. **风险度量主线**：波动率、下行风险、夏普/索提诺/卡尔玛、最大回撤，构成最常见的风险收益评估框架。

**理解即可**

4. **分布特征**：偏度和峰度解释了为什么金融收益常常“左尾更重、极端更多”。
5. **尾部风险**：VaR 给出损失门槛，ES 则进一步回答“最坏情况下平均会亏多少”；在厚尾环境下，ES 通常比 VaR 更稳健。
6. **系统性风险**：Beta 是理解 CAPM 与因子模型的基础桥梁。

**实践提醒**

任何风险指标都不是万能尺子。实务里应把收益、波动、回撤、尾部风险和成本一起看，而不是只盯一个漂亮的夏普比率。
...
!!! note "使用建议"
    建议按“收益率与基础风险 → 尾部风险 → 综合讨论”顺序完成本章习题。若是本科主线课程，可重点完成 5.1、5.2、5.4；若是进阶课程或风险管理方向，可进一步完成 5.3、5.5。
