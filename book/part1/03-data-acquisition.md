...
!!! example "例 3.1：10 送 10 派现 2 元的完整复权数字算例"
    ...
    **第一步：计算除权价（理论开盘价）**
    ...
    $$
    P_{\text{除权}} = \frac{P_{T-1} - \text{现金红利}}{1 + \text{送股比例}}
    = \frac{20.00 - 2.00}{1 + 1} = \frac{18.00}{2} = 9.00 \text{ 元}
    $$
...
!!! example “例 3.3：3σ 法与 MAD 法的异常值检测对比数字算例”
    ...
    **方法一：3σ 法（均值±3倍标准差）**
    ...
    $$
    \bar{r} = \frac{-1.2+2.3+\cdots+1.8}{10} = -3.22\%
    \quad (\text{被 } {-38\%} \text{ 严重拉低})
    $$

    $$
    \sigma = 11.78\%
    \quad (\text{被 } {-38\%} \text{ 严重拉大})
    $$

    $$
    \text{上界} = -3.22 + 3\times11.78 = 32.12\%,\quad
    \text{下界} = -3.22 - 3\times11.78 = -38.56\%
    $$
    ...
    **方法二：MAD 法（中位数绝对偏差，$k = 3.5$）**
    ...
    $$
    \text{median}(r) = \frac{0.9 + 1.5}{2} = 1.05\%
    \quad (\text{对极端值鲁棒})
    $$
