...
本章建立了贯穿全书的工具基础。学完后，建议按以下三层来掌握：

**必须掌握**

1. **环境复现**：通过 `pyproject.toml` + `uv.lock` 管理依赖，是后续所有实验可重复的前提。
2. **数值计算**：NumPy 的向量化、广播和矩阵运算，是金融计算提速与避免循环错误的基础。
3. **时间序列处理**：Pandas 的 `DatetimeIndex`、索引切片、`resample`、`rolling`、`ewm` 构成金融数据处理主工具链。

**理解即可**

4. **存储格式**：研究与内部处理中优先 Parquet，对外交换时再使用 CSV。
5. **绘图基础**：能够稳定输出带中文标题、图例和保存文件的图表，就足以支撑本书大多数实验。

**实践提醒**

写得更快不等于写得更对。金融计算里优先使用向量化、显式时间索引和可复现环境，远比“先跑通”更重要。
...
!!! note "使用建议"
    建议按“数值计算 → 时间序列处理 → 文件与图表输出”顺序完成本章习题。若是本科主线课程，可重点完成 1、2、3、5 题；第 4 题更强调工程习惯。
