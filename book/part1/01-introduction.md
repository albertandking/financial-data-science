...
假设你用2015—2023年A股日度数据回测一个“买入涨停股翌日开盘卖出”的策略。请列举至少三个可能导致回测虚高的数据处理错误，并说明如何修正。
...
EMH弱式有效意味着历史价量数据不能用于预测超额收益。但机器学习研究确实发现了若干A股量价因子的预测力（IC值不为零）。请讨论：这两个观察是否矛盾？至少给出两种解释，说明预测力为何可以同时存在于“接近弱式有效”的市场中。
